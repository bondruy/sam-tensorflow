...
def train_model(dataset, paths, device):
    ...
    input_images, ground_truths_maps, ground_truths_fixations = next_element[:3]
    ...
    with tf.Session() as sess:
        ...
        saver = sam_net.restore(sess, dataset, paths, device)
        ...
        for epoch in range(config.PARAMS["n_epochs"]):
            ...
            sam_net.save(saver, sess, dataset, paths["latest"], device)
            ...
            if history.valid_history[-1] == min(history.valid_history):
                sam_net.save(saver, sess, dataset, paths["best"], device)
                sam_net.optimize(sess, dataset, paths["best"], device)
...
def test_model(dataset, paths, device):
    ...
    next_element, init_op, test_data_n = iterator
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            _paths.append(path)
            _images.append(output_file)
        print("fps: {}".format(test_data_n / (time.time() - start_time)))

        os.makedirs(paths["images"], exist_ok=True)
        for _img, _path in zip(_images, _paths):
            filename = os.path.basename(_path)
            filename = os.path.splitext(filename)[0]
            filename += ".png"
            # cv2.imwrite(paths["images"] + filename, _img.astype(int))
            cv2.imwrite(paths["images"] + filename, _img)
