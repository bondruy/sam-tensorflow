...
class SALICON:
    ...
    def __init__(self, data_path):
        self._target_size = config.DIMS["image_in_size_salicon"]
        self._out_size = config.DIMS["image_out_size_salicon"]

        self._dir_stimuli_train = data_path + "images/train"
        self._dir_stimuli_valid = data_path + "images/val"

        self._dir_saliency_train = data_path + "maps/train"
        self._dir_saliency_valid = data_path + "maps/val"

        self._dir_fixation_train = data_path + "fixations1/train"
        self._dir_fixation_valid = data_path + "fixations1/val"
    ...
    def load_data(self):
        ...
        train_list_y_maps = _get_file_list(self._dir_saliency_train)
        train_list_y_fixations = _get_file_list(self._dir_fixation_train)

        _check_consistency(zip(train_list_x, train_list_y_maps, train_list_y_fixations), 10000)

        train_set = _fetch_dataset((train_list_x, train_list_y_maps, train_list_y_fixations),
                                   self._target_size, self._out_size, True)

        valid_list_x = _get_file_list(self._dir_stimuli_valid)
        valid_list_y_maps = _get_file_list(self._dir_saliency_valid)
        valid_list_y_fixations = _get_file_list(self._dir_fixation_valid)

        _check_consistency(zip(valid_list_x, valid_list_y_maps, valid_list_y_fixations), 5000)

        valid_set = _fetch_dataset((valid_list_x, valid_list_y_maps, valid_list_y_fixations),
                                   self._target_size, self._out_size, False)
...
class TEST:
    ...
    def __init__(self, dataset, data_path):
        self._target_size = config.DIMS["image_in_size_%s" % dataset]
        self._out_size = config.DIMS["image_out_size_%s" % dataset]
    ...
    def load_data(self):
        ...
        self.test_data_n = len(test_list_x)

        test_set = _fetch_dataset(test_list_x, self._target_size, self._out_size,
                                  False, online=True)
        return test_set

    def data_len(self):
        return self.test_data_n
...
def get_dataset_iterator(phase, dataset, data_path):
    ...
    if phase == "test":
        ...
        test_set_n = test_class.data_len()
        ...
        return next_element, init_op, test_set_n
...
def postprocess_saliency_map(saliency_map, target_size):
    ...
    # saliency_map_png = tf.image.encode_png(saliency_map)

    return saliency_map


def _fetch_dataset(files, target_size, out_size, shuffle, online=False):
    ...
    dataset = dataset.map(lambda *files: _parse_function(files, target_size, out_size),
                          num_parallel_calls=tf.data.experimental.AUTOTUNE)

    batch_size = config.PARAMS["batch_size_test"] if online else config.PARAMS["batch_size"]
...
def _parse_function(files, target_size, out_size):
    ...
    for count, filename in enumerate(files):
        ...
        size = target_size if count == 0 else out_size
